...
export default function GrantSearchClient() {
  ...
  // Function to handle checkbox changes for type filters
  const handleTypeChange = (key: keyof TypeFilters) => {
    setFilters((prev) => ({
      ...prev,
      type: {
        ...prev.type,
  ...
  // Function to handle checkbox changes for amount filters
  const handleAmountChange = (key: keyof AmountFilters) => {
    setFilters((prev) => ({
      ...prev,
      amount: {
        ...prev.amount,
  ...
  // Function to handle checkbox changes for deadline filters
  const handleDeadlineChange = (key: keyof DeadlineFilters) => {
    setFilters((prev) => ({
      ...prev,
      deadline: {
        ...prev.deadline,
        [key]: !prev.deadline[key],
      },
    }));
  };

  // Function to handle all checkbox changes
  const handleCheckboxChange = (
    category: keyof Filters,
    key: string
  ) => {
    if (category === "type") {
      handleTypeChange(key as keyof TypeFilters);
    } else if (category === "amount") {
      handleAmountChange(key as keyof AmountFilters);
    } else if (category === "deadline") {
      handleDeadlineChange(key as keyof DeadlineFilters);
    }
  };
